...
const ButtonPagination = ({ pageNumber, currentPage, totalPages }) => {
  ...
  return (
   ...
   <button className="button-wrapper wrapper-flex padding-button" onClick={() => undoListAndNavigate(pageNumber)} disabled={currentPage == 1}>
    <ion-icon class="icon tiny" name="arrow-undo"></ion-icon>
    <p className="tiny-bold">UNDO</p>
   </button>
   <div className="button-wrapper tiny-bold padding-button">PAGE {pageNumber} OF {totalPages}</div>
   <button className="button-wrapper wrapper-flex padding-button" onClick={() => showListAndNavigate(pageNumber)} disabled={currentPage >= totalPages}>
